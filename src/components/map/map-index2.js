...
export default function MapIndex2({ 
    ...
    // This function fetches the NLCD service and stores it in a WMSLayer object. It is toggled off by default
    const drawLandUse = () => {
        return new Promise((resolve, reject) => {
            if (mapRef) {
                ...
                .then(([WMSLayer]) => {
                    landUseLayerRef.current = new WMSLayer({
                        id: 'nlcd-layer',
                        title: '2024 National Land Cover Database',
                        url: 'https://dmsdata.cr.usgs.gov/geoserver/mrlc_Land-Cover-Native_conus_year_data/wms?',
                        sublayers: [{
                            name: 'Land-Cover-Native_conus_year_data',
                            title: 'Land Cover',
                            // Legend image is fetched on the fly and generated by the mrlc server
                            // The 2024 service does not have an associated legend graphic url, so using the 2021 legend for now. Numbers are the same. Colors appear to be the same except for maybe Grassland/Herbaceous (71)
                            legendUrl: 'https://www.mrlc.gov/geoserver/mrlc_display/wms?REQUEST=GetLegendGraphic&FORMAT=image/png&WIDTH=12&HEIGHT=12&LAYER=NLCD_2021_Land_Cover_L48&legend_options=fontAntiAliasing:true;fontSize:8;dpi:100'
                        }],
                        copyright: 'MRLC NLCD',
