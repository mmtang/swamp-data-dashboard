...
export default function MapSpot({ 
    ...
    useEffect(() => {
        if (stationLayerRef.current) {
            ...
            // Change title of station layer
            // stationLayerRef.current.title = analyte ? `SPoT Monitoring Sites - ${analyte.name}` : 'SPoT Monitoring Sites';
        }
    }, [stationData]);
